...
contract BloodBank {
    ...
    modifier onlyAdminOrPatient() {
        require(
            msg.sender == admin || allowedPatients[msg.sender],
            "Permission denied"
        );
        _;
    }
    ...
    function grantPatientPermission(address patientAddress) public {
        require(
            !allowedPatients[patientAddress],
            "Patient already has permission"
        );
    ...
    function addHospital(
        ...
        require(
            !hospitals[hospitalAddress].isRegistered,
    ...
    function locateHospitalToDonate(
        address hospitalAddress
    ) public view returns (string memory name, address location) {
        require(
            hospitals[hospitalAddress].isRegistered,
            "Hospital not registered"
        );
        return (
            hospitals[hospitalAddress].name,
            hospitals[hospitalAddress].location
        );
    }
    ...
    function respondToRequest(
        ...
        require(
            patientRequests[patientAddress][bloodType].length > 0,
            "No blood donation request found"
        );

        // Get the last request made by the patient
        Request storage lastRequest = patientRequests[patientAddress][
            bloodType
        ][patientRequests[patientAddress][bloodType].length - 1];
    ...
    function getRegisteredUsers()
        ...
        view
        returns (
            address[] memory registeredDonors,
            address[] memory registeredPatients
        )
    {
