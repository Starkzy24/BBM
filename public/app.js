...
const contractAddress = "0x8A4353da29Ee9246662f5366DCB89200F52A9F77";
const contractABI = [
  {
    inputs: [],
...
async function registerUser() {
  try {
    ...
    const selectedBloodType =
      bloodTypeSelect.options[bloodTypeSelect.selectedIndex].value;
    ...
    const bloodTypeNumericValue =
      bloodTypeMapping[selectedBloodType.toUpperCase()];
    ...
    const defaultAccount = await web3.eth
      .getAccounts()
      .then((accounts) => accounts[0]);

    // Call the appropriate registration method in your smart contract
    const transaction = await registrationMethod(
      userName,
      bloodTypeNumericValue
    ...
    if (error.message.includes("insufficient funds")) {
      alert(
        "Insufficient funds. Please make sure your account has enough ETH."
      );
  ...
  try {
    ...
    if (isDonor) {
      await bloodBankContract.methods
        .grantDonorPermission(userAddress)
        .send({ from: userAddress });
      alert("Donor Permission Granted");
    } else if (isPatient) {
      await bloodBankContract.methods
        .grantPatientPermission(userAddress)
        .send({ from: userAddress });
  ...
  try {
    ...
    for (const patientAddress of registeredPatients) {
      ...
      const requests = await bloodBankContract.methods
        .getPatientRequests(patientAddress, bloodType)
        .call();
...
async function respondToRequest() {
  try {
    ...
    const response = confirm(
      "Do you want to approve the blood donation request?"
    );

    // Call the smart contract function for responding to blood donation requests
    await bloodBankContract.methods
      .respondToRequest(patientAddress, bloodType, response)
      .send({ from: userAddress });
...
async function displayPatientResponses() {
  try {
    ...
    const patientsWithResponses = await Promise.all(
      registeredPatients.map(async (patientAddress) => {
        const responses = await bloodBankContract.methods
          .getPatientResponses(patientAddress)
          .call();
    ...
    const patientsWithResponsesFiltered = patientsWithResponses.filter(
      (patient) => patient.hasResponses
    );
    ...
    const selectedPatient = prompt(
      "Select a patient with responses:\n" +
        patientsWithResponsesFiltered
          .map((patient, index) => `${index + 1}. ${patient.patientAddress}`)
          .join("\n")
    );
    ...
    if (
      !selectedPatient ||
      isNaN(selectedPatient) ||
      selectedPatient < 1 ||
      selectedPatient > patientsWithResponsesFiltered.length
    ) {
    ...
    const chosenPatient =
      patientsWithResponsesFiltered[selectedPatient - 1].patientAddress;
    const responses = await bloodBankContract.methods
      .getPatientResponses(chosenPatient)
      .call();
